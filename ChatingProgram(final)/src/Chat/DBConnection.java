...
public class DBConnection {
	...
	public void CreateTable() {
		try {
			SQL = "CREATE TABLE "+TableName 
					...
					+ "Birth VARCHAR(20) NOT NULL,Email VARCHAR(30) NOT NULL,"
					+ "PhoneNum VARCHAR(13),"
	...
	public void createAccount(String ID,String NickName,String Name,String Birth,
							String Email,String PhoneNum,String StatusMessage,String PW) {
		//계정 생성. DB에 입력할 필수 요소들을 입력받는다.
		try {
			SQL = "INSERT INTO "+TableName+"(ID,NickName,Name,Birth,Email,PhoneNum,StatusMessage,PW) "
					+"VALUES ('"+ID+"','"+NickName+"','"+Name+"','"+Birth+"','"+Email+"','"+PhoneNum+"','"
					+StatusMessage+"', HEX(AES_ENCRYPT('"+PW+"','"+ID+"')))";
			st.executeUpdate(SQL);
	...
	public boolean cancelAccount(String ID, String PW, String Email) { // 계정 삭제.
		...
		try {
			...
			if(rs.next()) {
				String[] ids = getIDList().split(" ");
				
				for (String id: ids) {
					SQL = "DELETE FROM "+id+"friends WHERE FriendID='"+ID+"'" ;
					st.executeUpdate(SQL);
				}
	...
	public String searchItems(String myID, String keyword) {
		String searchId="";
		String searchName=""; //name으로 검색한 아이디
		String searchNickname=""; //nickname으로 검색한 아이디
		String searchResult="";
		try {
			SQL = "SELECT ID FROM "+TableName+" WHERE ID LIKE '%"+keyword+"%'"; //search in ID
			rs=st.executeQuery(SQL);
			while(rs.next()) {searchId += (rs.getString("ID")+",/");}
			
			SQL = "SELECT ID FROM "+TableName+" WHERE Name LIKE '%"+keyword+"%'"; //search in Name
			rs=st.executeQuery(SQL);
			while(rs.next()) {searchName += (rs.getString("ID")+",/");}
				
			SQL = "SELECT ID FROM "+TableName+" WHERE Nickname LIKE '%"+keyword+"%'"; //search in NickName
			rs=st.executeQuery(SQL);			
			while(rs.next()) {searchNickname += (rs.getString("ID")+",/");}			
			
			searchResult = searchId+searchName+searchNickname;
	...
	public String searchFriends(String myID, String keyword) {
		String searchId="";
		String searchName=""; //name으로 검색한 아이디
		String searchNickname=""; //nickname으로 검색한 아이디
		String searchResult="";
		String[] flist = getFriendList(myID).split(" ");
		try {
			SQL = "SELECT * FROM "+myID+"friends WHERE FriendID LIKE '%"+keyword+"%'"; //search in ID
			rs=st.executeQuery(SQL);
			while(rs.next()) {searchId += (rs.getString("FriendID")+",/");}
			
			for (String frid:flist) {
				SQL = "SELECT * FROM "+TableName+" WHERE Name LIKE '%"+keyword+"%' and ID='"+frid+"'"; //search in Name
				rs=st.executeQuery(SQL);
				while(rs.next()) {searchName += (rs.getString("ID")+",/");}
				
				SQL = "SELECT * FROM "+TableName+" WHERE Nickname LIKE '%"+keyword+"%'and ID='"+frid+"'"; //search in NickName
				rs=st.executeQuery(SQL);			
				while(rs.next()) {searchNickname += (rs.getString("ID")+",/");}
			}
			searchResult = searchId+searchName+searchNickname;
	...
	public String getIDList() { //ID 검색
		String flist="";
		try {
			SQL = "SELECT * FROM "+TableName;
			rs = st.executeQuery(SQL);
			while(rs.next()) {
				 flist += (rs.getString("ID")+" ");
			}
		} catch (Exception e) {
			e.printStackTrace();
		} 
	...
	public void deleteFriend(String myID, String FrID) {
		try {
			SQL = "DELETE FROM "+myID+"friends WHERE FriendID='"+FrID+"'" ;
			st.executeUpdate(SQL);
			SQL = "DELETE FROM "+FrID+"friends WHERE FriendID='"+myID+"'" ;
			st.executeUpdate(SQL);
	...
	public String findID(String Birth, String Email, String PhoneNum) {
		String ID = null;
		try {
			SQL = "SELECT ID FROM "+TableName+" WHERE Birth='"+Birth+"' and Email='"+Email+"' and PhoneNum = '"+PhoneNum+"'";
			rs = st.executeQuery(SQL);
			if(rs.next()) {
				ID = rs.getString("ID");
			}
	...
	public String findPW(String ID, String Email, String PhoneNum) {
		String pw = null;
		try {
